            ...
            try:
                ...
                try:
                    ...
                    return JsonResponse({'status': 'success', 'message': 'Increased cart Quantity',
                                         'cart_counter': get_cart_counter(request), 'qty': cart_item.quantity})
                except:
                    ...
                    return JsonResponse({'status': 'success', 'message': 'Added Food to Cart',
                                         'cart_counter': get_cart_counter(request), 'qty': 1})
            except:
        ...
        if request.headers.get('x-requested-with') == "XMLHttpRequest":
            ...
            try:
                ...
                try:
                    ...
                    return JsonResponse(
                        {'status': 'success', 'cart_counter': get_cart_counter(request), 'qty': cart_item.quantity})
                except:
                    Cart.objects.create(user=request.user, fooditem=fooditem, quantity=1)
